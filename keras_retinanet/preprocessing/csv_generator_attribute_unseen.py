...
class CSVGenerator(Generator):
    ...
    def word_data(self):
        if self.word_data_type == 'attribute':
            # word_number = 65
            # word_shape = 79
            word_data = np.loadtxt(r'MSCOCO/attribute.txt', dtype='float32',
                                   delimiter=',')
            word_data = np.transpose(word_data[:, :65])
        elif self.word_data_type == 'attribute_unseen':
            # word_number =15
            # word_shape = 79
            word_data = np.loadtxt(r'MSCOCO/attribute.txt', dtype='float32',
                                   delimiter=',')
            word_data = np.transpose(word_data[:, 65:])
        elif self.word_data_type == 'attribute_all':
            # word_number = 80
            # word_shape = 79
            word_data = np.loadtxt(r'MSCOCO/attribute.txt', dtype='float32',
                                   delimiter=',')
    ...
    def compute_targets(self, image_group, annotations_group):
        ...
        # iou = 0.7
        labels_word_group = [None] * self.batch_size
        for index, (image, annotations_base) in enumerate(zip(image_group, annotations_group)):
            # compute regression targets
            labels_group[index], annotations, anchors = self.anchor_targets(max_shape, annotations_base, self.num_classes(),
            ...
            # iou = 0.7
            labels_word_group[index], _, _ = self.anchor_targets(max_shape, annotations_base, self.num_classes(),
                                                                 positive_overlap=0.7, mask_shape=image.shape)
        ...
        labels_word_batch = np.zeros((self.batch_size,) + labels_word_group[0].shape, dtype=keras.backend.floatx())

        # copy all labels and regression values to the batch blob
        for index, (labels, regression, labels_word) in enumerate(
                zip(labels_group, regression_group, labels_word_group)):
            labels_batch[index, ...] = labels
            regression_batch[index, ...] = regression
            labels_word_batch[index, ...] = labels_word

        return [regression_batch, labels_batch, labels_word_batch]
